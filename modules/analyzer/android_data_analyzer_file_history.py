...
ANALYSIS_EXCEPTION_CALL_HISTORY = os.getcwd() + os.sep + 'config' + os.sep + 'ANALYSIS_EXCEPTION_FILE_HISTORY.conf'

class FileHistory(object):
	def get_logs_from_pp_db(case):
		query = "SELECT DISTINCT pp.cnt_records, di.id_package, di.inode, di.parent_path, di.name, pp.table_name, pp.cnt_timestamp, pp.cnt_file, pp.cnt_phonenumber, pp.cnt_account, pp.cnt_contents, pp.timestamp, pp.file, pp.phonenumber, pp.account, pp.contents, di.extracted_path\
			FROM sqlitedb_info as di, package_info as pi, sqlitedb_table_preprocess as pp\
			WHERE di.inode = pp.inode and (pp.cnt_timestamp >= 1 and pp.cnt_file >= 1)"
...
#---------------------------------------------------------------------------------------------------------------
	def do_analyze(list_userinfo_record, case, result):
		...
		for userinfo_record in list_userinfo_record:
			dic_userinfo_record = {'id_package':0, 'package_name':"", 'inode':0, 'parent_path':"", 'db_name':"", 'table_name':"", 'cnt_records':0, 'timestamp':"", 'phonenumber':"", 'account':"", 'contents':"", 'file':"", 'db_path':""}
			dic_userinfo_cnt = {'timestamp':0, 'phonenumber':0, 'account':0, 'contents':0, 'file':0}
			...
			dic_userinfo_cnt['file'] 			= userinfo_record[7]
			dic_userinfo_cnt['phonenumber'] 	= userinfo_record[8]
			dic_userinfo_cnt['account']			= userinfo_record[9]
			dic_userinfo_cnt['contents'] 		= userinfo_record[10]

			dic_userinfo_record['timestamp'] 	= userinfo_record[11]
			dic_userinfo_record['file']			= userinfo_record[12]
			dic_userinfo_record['phonenumber'] 	= userinfo_record[13]
			dic_userinfo_record['account'] 		= userinfo_record[14]
			dic_userinfo_record['contents'] 	= userinfo_record[15]
			...
			table_name_to_insert = "file_history"
			not_null_userinfo_type = "file"
...
#---------------------------------------------------------------------------------------------------------------
	def analyze_with_preprocess_db(case):
		...
		if length < NUMBER_OF_PROCESSES:
			result = Queue()
			FileHistory.do_analyze(list_userinfo_record, case, result)
		else:
			...
			for i in range(len(divied_list)):
				proc = Process(target=FileHistory.do_analyze, args=(divied_list[i], case, result))
				procs.append(proc)
				proc.start()
