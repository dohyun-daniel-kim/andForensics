...
ANALYSIS_EXCEPTION_CALL_HISTORY = os.getcwd() + os.sep + 'config' + os.sep + 'ANALYSIS_EXCEPTION_WEB_BROWSER_HISTORY.conf'

class WebBrowserHistory(object):
	def get_logs_from_pp_db(case):
		query = "SELECT DISTINCT pp.cnt_records, di.id_package, di.inode, di.parent_path, di.name, pp.table_name, pp.cnt_timestamp, pp.cnt_url, pp.cnt_account, pp.cnt_digit_positive, pp.cnt_file, pp.cnt_contents, pp.timestamp, pp.url, pp.account, pp.digit_positive, pp.file, pp.contents, di.extracted_path\
			FROM sqlitedb_table_preprocess as pp, sqlitedb_info as di\
			WHERE pp.inode = di.inode and pp.cnt_url >= 1 and (pp.cnt_phonenumber = 0 and pp.cnt_mac = 0)"
		...
		if (list_userinfo_record == False) | (list_userinfo_record == []):
			logger.error('The image has no web browser history information.')
			return False
...
#---------------------------------------------------------------------------------------------------------------
	def do_analyze(list_userinfo_record, case, result):
		...
		for userinfo_record in list_userinfo_record:
			dic_userinfo_record = {'id_package':0, 'package_name':"", 'inode':0, 'parent_path':"", 'db_name':"", 'table_name':"", 'cnt_records':0, 'timestamp':"", 'account':"", 'url':"", 'digit_positive':"", 'contents':"", 'file':"", 'db_path':""}
			dic_userinfo_cnt = {'timestamp':0, 'account':0, 'url':0, 'digit_positive':0, 'contents':0, 'file':0}
			...
			dic_userinfo_cnt['url'] 			= userinfo_record[7]
			dic_userinfo_cnt['account'] 		= userinfo_record[8]
			dic_userinfo_cnt['digit_positive'] 	= userinfo_record[9]
			dic_userinfo_cnt['file'] 			= userinfo_record[10]
			dic_userinfo_cnt['contents'] 		= userinfo_record[11]

			dic_userinfo_record['timestamp'] 		= userinfo_record[12]
			dic_userinfo_record['url'] 				= userinfo_record[13]
			dic_userinfo_record['account'] 			= userinfo_record[14]
			dic_userinfo_record['digit_positive'] 	= userinfo_record[15]
			dic_userinfo_record['file']				= userinfo_record[16]
			dic_userinfo_record['contents'] 		= userinfo_record[17]
			dic_userinfo_record['db_path'] 			= userinfo_record[18]
			...
			table_name_to_insert = "web_browser_history"
			not_null_userinfo_type = "url"
...
#---------------------------------------------------------------------------------------------------------------
	def analyze_with_preprocess_db(case):
		list_userinfo_record = WebBrowserHistory.get_logs_from_pp_db(case)
		if (list_userinfo_record == []) | (list_userinfo_record == False):
			logger.error('There are no web browser history to analyze.')
			return False
		...
		if length < NUMBER_OF_PROCESSES:
			result = Queue()
			WebBrowserHistory.do_analyze(list_userinfo_record, case, result)
		else:
			...
			for i in range(len(divied_list)):
				proc = Process(target=WebBrowserHistory.do_analyze, args=(divied_list[i], case, result))
				procs.append(proc)
				proc.start()
